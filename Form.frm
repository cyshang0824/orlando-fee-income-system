...
Sub 儲存與印收據_Click()
    ...
    If Left(ws紀錄_External.Cells(Max - 1, 10), 2) = "PC" Then
        PreviousNumber = Val(Mid(ws紀錄_External.Cells(Max - 1, 10), 3)) + 1
        ws紀錄_External.Cells(Max, 10) = "PC" & Format(PreviousNumber, "0000")
    Else
        ws紀錄_External.Cells(Max, 10) = "PC0001"
    End If
    ...
    backupPath = folderPath & "住戶總表_" & timestamp & ".xlsx"
...
Sub 刪除舊備份檔案()
    ...
    fileName = Dir(folderPath & "住戶總表_????????_??????.xlsx")
    Do While fileName <> ""
        timestamp = Mid(fileName, 6, 15)
        dateTimeString = Left(timestamp, 4) & "/" & Mid(timestamp, 5, 2) & "/" & Mid(timestamp, 7, 2) & " " & Mid(timestamp, 10, 2) & ":" & Mid(timestamp, 12, 2) & ":" & Right(timestamp, 2)
        On Error Resume Next
        currentTimestamp = CDbl(CDate(dateTimeString))
